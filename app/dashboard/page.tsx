...
import { createChatSession, getAllChatSessions } from "@/lib/api/chat";
...
export default function Dashboard() {
  ...
  const fetchDailyStats = useCallback(async () => {
    try {
      ...
      if (!token) {
        console.warn("No token found. Skipping daily stats fetch.");
        // You might want to throw an error or handle unauthorized display here
        return;
      }
      ...
      const activitiesResponse = await authenticatedFetch(
        "http://localhost:3001/api/activity/today",
        router
      );
  ...
  // Refactored handler to prevent double-click
  const handleStartTherapy = async () => {
    ...
    try {
      ...
      if (newSession) {
        router.push(`/therapy/${newSession}`);
      } else {

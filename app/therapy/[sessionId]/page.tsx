...
export default function TherapyPage() {
  ...
  const refreshSessions = useCallback(async () => {
    try {
      ...
      setSessions(allSessions); // sessions always come from backend
    } catch (error) {
  ...
  const handleNewSession = async () => {
    try {
      ...
      // Switch active session to new one
  ...
  useEffect(() => {
    const initChat = async () => {
      try {
        ...
        // Always fetch sessions from backend
        const allSessions = await getAllChatSessions();
        setSessions(allSessions);

        let targetSessionId = sessionId;

        // If no session ID is set (or invalid), fall back to backend truth
        if (!targetSessionId || targetSessionId === "new") {
          if (allSessions.length > 0) {
            targetSessionId = allSessions[0].sessionId; // latest from backend
          } else {
            targetSessionId = await createChatSession(); // only create if none exist
            await refreshSessions(); // immediately refresh after creation
          }

          setSessionId(targetSessionId);
          window.history.pushState({}, "", `/therapy/${targetSessionId}`);
        }
  ...
  // Responsible for sending user messages to the AI and processing the AI's response
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, userMessage]); // Update local messages state with new message

      await refreshSessions(); // Refresh the entire session list from the server
      ...
      // Update local messages state with the AI's message immediately
  ...
  // This runs when a user clicks on a session on the sidebar
  const handleSessionSelect = async (selectedSessionId: string) => {
    ...
    try {
      ...
      const history = await getChatHistory(selectedSessionId); // Get chat history for the selected session ID

      if (Array.isArray(history)) {
        // If the history is an array, format the timestamps of each message into a date object
        const formattedHistory = history.map((msg) => ({
          ...msg,
          timestamp: new Date(msg.timestamp),
        }));
        setMessages(formattedHistory); // Update local messages state to the formatted message history
        setSessionId(selectedSessionId); // Update local session ID state to the selected session's ID
        window.history.pushState({}, "", `/therapy/${selectedSessionId}`); // Update browser url
      }
    } catch (error) {
